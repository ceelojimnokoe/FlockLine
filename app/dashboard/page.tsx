...
import {
  getCurrentChurchUser,
  canViewGiving,
  getCurrentUserDisplayName,
} from "@/lib/data/church";
...
export default async function DashboardPage() {
  ...
  const [summary, displayName, needsAttention, monthGivingTotal, totalMembers] =
    await Promise.all([
      getWeekSummary(),
      getCurrentUserDisplayName(),
  ...
  // list is empty, which is an honest degrade here.
  const [upcomingSessionsResult, prayerRequestsResult] =
  ...
  if (upcomingSessionsResult.status === "rejected") {
    console.error(
      "[dashboard] Failed to load upcoming sessions:",
      upcomingSessionsResult.reason,
    );
  }
  if (prayerRequestsResult.status === "rejected") {
    console.error(
      "[dashboard] Failed to load recent prayer requests:",
      prayerRequestsResult.reason,
    );
  }

  const upcomingSessions =
    upcomingSessionsResult.status === "fulfilled"
      ? upcomingSessionsResult.value
      : [];
  const prayerRequests =
    prayerRequestsResult.status === "fulfilled"
      ? prayerRequestsResult.value
      : [];
